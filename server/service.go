...
const (
	API_KEY = "AIzaSyA0H2gmjFao_osUOe6ZXJqZDTUO6iOuwEQ"
)
func init() {
...
func registerCloudEndpoints() {
	...
	info.Name, info.HttpMethod, info.Path, info.Desc = "Echo", "POST", "Echo", "Echoes your message."

	// Register Send method
	info = api.MethodByName("Send").Info()
	info.Name, info.HttpMethod, info.Path, info.Desc =	"Send", "POST", "Send", "Sends your message to the specified device."
...
func createRequest(msg *Message) (*http.Request, error) {
	...
	req, err := http.NewRequest("POST", "https://android.googleapis.com/gcm/send", body)
	if err != nil {
		return nil, err
	}
	// Add Auth headers
